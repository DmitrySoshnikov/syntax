...
describe('sets-generator', () => {
  ...
  describe('frist', () => {
    ...
    it('direct', () => {
      ...
      expect(setsGenerator.firstOf(new GrammarSymbol(`'a'`)))
        .toEqual({"'a'": true});

      expect(setsGenerator.firstOf(new GrammarSymbol(`'b'`)))
        .toEqual({"'b'": true});
    });
    ...
    it('RHS', () => {
      ...
      expect(setsGenerator.firstOfRHS(RHS))
        .toEqual({"'a'": true, "'b'": true, "'c'": true});

      // S -> D
      RHS = grammar.getProduction(2).getRHS();
      expect(setsGenerator.firstOfRHS(RHS))
        .toEqual({"'d'": true, 'ε': true});
    });
  ...
  describe('follow', () => {
    ...
    it('start symbol has $', () => {
      ...
      expect(setsGenerator.followOf(new GrammarSymbol('S')))
        .toEqual({'$': true});
    });
    ...
    it('single symbol', () => {
      ...
      let RHS = grammar.getProduction(1).getRHS();
      expect(setsGenerator.followOf(new GrammarSymbol('A')))
        .toEqual(setsGenerator.firstOfRHS(RHS.slice(1)));
    });
